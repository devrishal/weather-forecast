package com.wfs.api.swagger;

import com.wfs.utility.vo.WeatherVO;
import com.wfs.utility.exception.WFError;
...
@ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "401", description = "User doesn't have sufficient credentials.", content = @Content(mediaType = APPLICATION_JSON_VALUE,
                schema = @Schema(implementation = WFError.class))),
        @ApiResponse(responseCode = "403", description = "User is not authorised.", content = @Content(mediaType = APPLICATION_JSON_VALUE,
                schema = @Schema(implementation = WFError.class))),
        @ApiResponse(responseCode = "429", description = "Too Many request sent, Please wait for sometime before requesting again.", content = @Content(mediaType = APPLICATION_JSON_VALUE,
                schema = @Schema(implementation = WFError.class))),
